...
export default function CareGroupDetailPage() {
  ...
  } = useCareGroupDetail(careGroupId)
  const { data: stakeholders = [], error: stakeholdersError } = useCareGroupStakeholders(careGroupId)
  const { data: careGroupHomes = [], error: homesError } = useHomes(careGroupId)
  const careGroupErrorMessage = careGroupError
    ? getApiErrorMessage(careGroupError, "Unable to load care group.")
    : null
  const stakeholdersErrorMessage = stakeholdersError
    ? getApiErrorMessage(stakeholdersError, "Unable to load stakeholders.")
    : null
  ...
  if (careGroupErrorMessage) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Unable To Load Care Group</h1>
          <p className="text-gray-500 mt-1">{careGroupErrorMessage}</p>
        </div>
