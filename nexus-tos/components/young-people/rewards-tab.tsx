...
const rewardColumns: { key: RewardColumnKey; label: string; filterable: boolean; filterType: "search" | "select" | "none" }[] = [
  { key: "id", label: "ID", filterable: true, filterType: "search" },
  { key: "youngPerson", label: "Young Person", filterable: true, filterType: "search" },
  { key: "rewardType", label: "Reward Type", filterable: true, filterType: "search" },
  { key: "points", label: "Points", filterable: true, filterType: "search" },
  { key: "awardedBy", label: "Awarded By", filterable: true, filterType: "search" },
  { key: "awardedAt", label: "Awarded At", filterable: true, filterType: "search" },
  { key: "status", label: "Status", filterable: true, filterType: "select" },
]

const defaultRewardColumns: RewardColumnKey[] = ["id", "youngPerson", "rewardType", "points", "awardedBy", "awardedAt", "status"]

const rewardStatusBadge: Record<string, { bg: string; text: string }> = {
  awarded: { bg: "bg-green-600", text: "text-white" },
  redeemed: { bg: "bg-blue-500", text: "text-white" },
  expired: { bg: "bg-gray-400", text: "text-white" },
}

export function RewardsTab() {
  const [visibleColumns, setVisibleColumns] = useState<RewardColumnKey[]>(defaultRewardColumns)
  ...
  const filtered = allItems.filter((item) => {
    for (const [key, value] of Object.entries(filters)) {
      ...
      else if (key === "rewardType") cellValue = item.rewardType
      else if (key === "points") cellValue = item.points.toString()
      else if (key === "awardedBy") cellValue = item.awardedBy
      else if (key === "awardedAt") cellValue = item.awardedAt
      else if (key === "status") {
  ...
  const toggleColumn = (col: RewardColumnKey) => {
  ...
  const resetGrid = () => {
    setVisibleColumns(defaultRewardColumns)
    setFilters({})
          ...
          <TableHeader>
            <TableRow className="bg-gray-50">
              ...
              {visibleColumnDefs.map((col) => (
                <TableHead
                  key={col.key}
                  className={`font-semibold text-gray-700 ${col.key === "id" ? "w-16" : ""} ${col.key === "points" ? "w-20 text-center" : ""} ${col.key === "status" ? "text-center" : ""}`}
                >
                  {col.label}
                </TableHead>
              ))}
              ...
              {visibleColumnDefs.map((col) => (
                <TableHead key={`filter-${col.key}`} className="py-1">
                  {col.filterType === "search" && (
                    <div className={`relative ${col.key === "id" || col.key === "points" ? "max-w-16" : ""}`}>
            ...
            {paginated.length === 0 ? (
              ...
              paginated.map((item, index) => (
                ...
                >
                  ...
                  {visibleColumnDefs.map((col) => (
                    <TableCell key={col.key} className={`py-3 ${col.key === "points" || col.key === "status" ? "text-center" : ""}`}>
                      {col.key === "id" && (
                      ...
                      {col.key === "rewardType" && (
                        <span className="text-sm text-gray-700">{item.rewardType}</span>
                      )}
                      {col.key === "points" && (
                        <span className="text-sm font-medium text-gray-700">{item.points}</span>
                      )}
                      {col.key === "awardedBy" && (
                        <span className="text-sm text-gray-700">{item.awardedBy}</span>
                      )}
                      {col.key === "awardedAt" && (
                        <span className="text-sm text-gray-500 whitespace-nowrap">{item.awardedAt}</span>
                      )}
                      {col.key === "status" && (
                        <Badge className={`${rewardStatusBadge[item.status]?.bg ?? "bg-gray-400"} ${rewardStatusBadge[item.status]?.text ?? "text-white"} hover:opacity-90`}>
