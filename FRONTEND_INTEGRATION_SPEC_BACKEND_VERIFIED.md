...
- [x] API client supports bearer auth + refresh rotation + single retry
- [x] Session store persists `session` object, not only token/user
- [x] Tenant switcher implemented using `/auth/switch-tenant`
- [x] Privileged MFA flow implemented (`challenge` -> `verify` -> retry)
- [x] Turnstile widget integrated and token sent as `x-captcha-token`
- [x] Public auth forms handle CAPTCHA-specific errors
- [x] Role-based UI gating aligned with matrix above
- [x] Task and approval UIs enforce non-privileged restrictions
- [x] Invite lifecycle UI implemented (create/list/revoke/accept)
- [x] Audit + security alerts views implemented for admin personas
